...
const CreateSubCategory = () => {
  ...
  useEffect(() => {
  api.get("/pamoja/api/categories").then(res => setCategories(res.data));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
  await api.post("/pamoja/api/subcategories", { name, categoryId: Number(categoryId) });
  ...
  return (
    <div className="py-40 flex items-start justify-center min-h-full">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-lg w-full max-w-sm space-y-6">
        <h2 className="text-2xl font-bold text-center text-rose-700 mb-4">Créer une sous-catégorie</h2>
        <div>
        ...
        <div>
          <label className="block mb-1 text-rose-700 font-semibold">Catégorie</label>
          <select value={categoryId} onChange={e => setCategoryId(e.target.value)} className="w-full px-4 py-2 border-2 border-rose-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-300" required>
